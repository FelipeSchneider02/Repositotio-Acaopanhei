...
import qrcode
from services import cadastrar_pet, dados_owners
...
class Create(ctk.CTkToplevel):
    ...
    def salvar(self):
        ...
        # verifica se o campo "sexo" não está "sexo"
        if self.sexo == "Sexo":
            self.erro_label.configure(text="Selecione o sexo do pet")
            return
        ...
        if cadastrar_pet(self.nome_pet, self.raca, self.sexo, self.obs, self.id_usuario_logado):
            ...
            # limpa os campos do pet
            self.limpar_campos()

            # retorna mensagem de sucesso
            self.erro_label.configure(text="Pet cadastrado com sucesso!")
...
%2AObservacoes%3A%2A {self.obs}%0A"""
        ...
        self.qr_nome = f"qrcodes/qrcode_cuidador_{nome_responsavel}_pet_{self.nome_pet}.png"
